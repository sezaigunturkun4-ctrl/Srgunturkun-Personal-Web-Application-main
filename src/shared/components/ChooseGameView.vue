  ...
  <div class="choose-game-view">
    ...
    <div class="game-filter">
        ...
        <div v-if="gameDisplayInfo && hasGameDetails" class="game-details-card">
          <div class="game-header">
            ...
            <div class="game-basic-info">
              ...
              <div class="game-meta">
                <span v-if="gameDisplayInfo.developer" class="meta-item">👨‍💻 {{ gameDisplayInfo.developer }}</span>
      ...
      <div v-if="gamesLoading" class="games-loading">
        ...
        <p>Loading games from Firebase...</p>
      </div>
  ...
  setup() {
    ...
    async function loadGameCategories() {
      try {
        ...
        const games = []
        const gameDocuments = [] // Store complete game documents with all fields
        
        snapshot.forEach((doc) => {
          const gameData = doc.data()
          
          // Create complete game document object with all fields
          const gameDocument = {
            // Firestore metadata
            docId: doc.id,
        ...
        // Store complete game documents globally for debugging and future use
    ...
    const hasGameDetails = computed(() => {
      return gameId.value || gameDescription.value || gameDeveloper.value || 
    ...
    // Load detailed game information from Firestore
    async function loadGameDropdown(selectedGameName) {
      ...
      try {
        // Query the games collection to find the game by name
        // Note: Firestore rules ensure only active games are visible to non-admin users
        const gameQuery = query(
          gamesCollection, 
          where('name', '==', selectedGameName)
        )
        
        const querySnapshot = await getDocs(gameQuery)
        
        if (!querySnapshot.empty) {
          // Get the first matching game document
          const gameDoc = querySnapshot.docs[0]
          const gameData = gameDoc.data()
          
          // Populate the basic game details (for compatibility)
          gameId.value = gameDoc.id
          gameName.value = gameData.name || ''
          gameDescription.value = gameData.description || ''
          gameDeveloper.value = gameData.developer || ''
          ...
          // Store complete game data for potential use by other components
          window.currentGameData = {
            docId: gameDoc.id,
            documentPath: gameDoc.ref.path,
            ...gameData
          }
          
        } else {
          // Clear details if game not found
          gameDescription.value = ''
          gameDeveloper.value = ''
          gameGenre.value = ''
          gameIconUrl.value = ''
          gameId.value = ''
          gameName.value = selectedGameName // Keep the name even if not found in DB
          gameRating.value = ''
          gameReleaseDate.value = ''
        ...
        // Clear global game data on error
        if (window.currentGameData) {
          delete window.currentGameData
        }
      }
    ...
    // Load game details by document ID (alternative method)
    async function loadGameById(gameDocId) {
      if (!gameDocId) return

      try {
        const gameDocRef = doc(gamesCollection, gameDocId)
